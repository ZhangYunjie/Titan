...
public:
    ...
    int validate(std::vector<b2Vec2> verticesVec)
    {
        ...
        for (; i< verticesVec.size(); i++)
        {
            i2 = i<verticesVec.size() - 1 ? i+1 : 0;
            i3 = i>0? i-1: verticesVec.size()-1;
            
            fl = false;
            int j = 0;
            for (; j < verticesVec.size(); j++)
            {
                if(j!=i&&j!=i2) {
                    if(!fl)
                    {
                        int d = det(verticesVec[i].x, verticesVec[i].y, verticesVec[i2].x, verticesVec[i2].y, verticesVec[j].x, verticesVec[j].y);
                        if (d>0) {
                            fl = true;
                        }
                    }
                    if (j!=i3) {
                        int j2 = (j<verticesVec.size()-1?j+1:0);
...
private:
    ...
    {
        int t1=x3-x1, t2=y3-y1, t3=x2-x1, t4=y2-y1, t5=x4-x3, t6=y4-y3;
        int t7=t4*t5-t3*t6, a;
    ...
    {
        bool b1 = ((x2+0.1>=px&&px>=x2-0.1)||(x1-0.1<=px&&px<=x2+0.1));
    ...
    {
        if(x2-x1>0.1||x1-px<0.1)
        {
            int a = (y2-y1)/(x2-x1);
            int possibleY = a*(px-x1)+y1;
            int diff = (possibleY>py?possibleY-py:py-possibleY);
    ...
    {
        return x1*x2+x2*y3+x3*y1-y1*x2-y2*x3-y3*x1;
    }
};
